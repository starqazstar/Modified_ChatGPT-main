...
const [pezzoInput, setPezzoInput] = useState("");

async function handlePezzoSubmit() {
  // Fetch the prompt from Pezzo
  const prompt = await pezzo.getPrompt(pezzoInput);

  // Use the OpenAI API to get the completion
  const response = await openai.chat.completions.create(prompt);

  // TODO: Send the response as a message in the chat interface
}
...
<div className={styles["chat-input-area"]}>
  <input 
    value={pezzoInput} 
    onChange={(e) => setPezzoInput(e.target.value)} 
    placeholder="输入简短的字符以获取Pezzo提示词" 
  />
  <button onClick={handlePezzoSubmit}>获取提示词</button>
</div>
